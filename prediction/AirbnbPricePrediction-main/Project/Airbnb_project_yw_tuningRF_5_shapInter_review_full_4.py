...
def prepare_dataset(df, feature, include_poi):
    ...
    print(df.columns)
    print(df.describe())
    # df.describe().to_csv(r'C:\Users\yw30f\OneDrive - University of Glasgow\LLM\data\rf_summary\featureDesc.csv')
...
def run_rf_full(df_model, save_prefix, base_dir):
    ...
    plot_shap_interaction_swapped(
        n_total_feature="n_total",
        poi_feature="poi_within_800m",
        shap_interaction_values=shap_inter,
        X_test=X_test,
        save_folder=f"{base_dir}/{prefix}/shap_interations"
    )
...
yr=2025
base_dir = rf"C:\Users\yw30f\OneDrive - University of Glasgow\LLM\data\rf_experiments_reviews_{yr}"
