...
def prepare_dataset(df, feature, include_poi):
    ...
    p90_reviews = df_model["number_of_reviews"].quantile(0.90)
    df_model = df_model[df_model["number_of_reviews"] <= p90_reviews]
...
def run_xgb_full(df_model, save_prefix, base_dir):
    ...
    # =====================================================
    # Advanced SHAP Interaction Plot (new improved version)
    # =====================================================
    ...
    print("Primary OK? ", "n_total" in X_test.columns)
    print("Interacting OK? ", "poi_within_800m" in X_test.columns)

    print("SHAP shape:", shap_inter.shape)
    ...
    # Correct SHAP slice
    shap_slice = shap_inter[:, f1, f2]

    # Data for plotting
    x_vals = X_test[primary_feature_name]
    interaction_vals = X_test[interacting_feature_name]

    save_folder = f"{base_dir}/{prefix}/shap_interations"
    os.makedirs(save_folder, exist_ok=True)

    plot_advanced_interaction_v5(
        primary_feature_name=primary_feature_name,
        interacting_feature_name=interacting_feature_name,
        x_values=x_vals,
        interaction_feature_values=interaction_vals,
        shap_interaction_slice=shap_slice,
    ...
    # plot_clean_shap_interaction_v2(
    #     primary_feature="n_total",
    #     interacting_feature="poi_within_800m",
