...
def run_rf_full(df_model, save_prefix, base_dir):
    ...
    sns.barplot(data=df_importance.head(30), x="importance", y="feature")
    plt.title("Random Forest Feature Importance (Top 30)")
    plt.savefig(f"{dirs['xgb']}/rf_importance_top30.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    # LIME
    save_lime_html(rf_model, X_test, dirs["lime"])

    # PDP + ICE
    save_pdp_ice(rf_model, X_train, top_features, dirs["pdp_ice"])

    return rmse_test, df_importance


for yr in [2022,2023,2024,2025]:
    base_dir = fr"C:\Users\yw30f\OneDrive - University of Glasgow\LLM\data\rf_experiments_{yr}"

    feature_list = ['none']#'none','n_total', 'n_generalParent']#'n_specific', 'n_total'
    poi_options = [ True]#False,f
    geojson_options = [False]#, True

    base_listings = pd.read_csv(
        fr"C:\Users\yw30f\OneDrive - University of Glasgow\LLM\data\P95_feature1_{yr}09.csv",
        index_col=0
    )
