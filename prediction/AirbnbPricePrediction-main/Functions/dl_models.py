...
##add new BNN
# ============================================================
#            PREPROCESSOR FOR TRAIN/VALID/TEST
# ============================================================
def prepare_bnn_data(train, valid, test):
    scaler = StandardScaler()
    scaler.fit(train[0])

    X_train = torch.tensor(scaler.transform(train[0]), dtype=torch.float32)
    y_train = torch.tensor(train[1].values, dtype=torch.float32).view(-1, 1)

    X_valid = torch.tensor(scaler.transform(valid[0]), dtype=torch.float32)
    y_valid = torch.tensor(valid[1].values, dtype=torch.float32).view(-1, 1)

    X_test = torch.tensor(scaler.transform(test[0]), dtype=torch.float32)
    y_test = torch.tensor(test[1].values, dtype=torch.float32).view(-1, 1)
...
#                        TRAINING LOOP
# ============================================================
def train_bnn_model(model, X_train, y_train, X_valid, y_valid, epochs=200):
    ...
    for epoch in range(epochs):
        ...
        model.eval()
        with torch.no_grad():
            preds = model(X_valid.to(device))
            val_rmse = np.sqrt(mean_squared_error(y_valid.cpu(), preds.cpu()))
        ...
        if epoch % 20 == 0:
            print(f"[Epoch {epoch}] Train Loss={total_loss:.3f}  Valid RMSE={val_rmse:.4f}")
...
def predict_bnn(model, X, samples=100):
    ...
    for _ in range(samples):
        preds.append(model(X.to(device)).cpu().detach())

    preds = torch.stack(preds)          # [samples, n, 1]
    mean_pred = preds.mean(0)
    epistemic = preds.var(0)
    aleatoric = preds.mean(0).var()

    return mean_pred.numpy(), epistemic.numpy(), aleatoric.numpy()
...
def BNN(train, valid, test):
    X_train, y_train, X_valid, y_valid, X_test, y_test, scaler = prepare_bnn_data(train, valid, test)

    model = BayesianRegressor(input_dim=X_train.shape[1])

    print("Training Bayesian Neural Network…")
    model, val_rmse = train_bnn_model(model, X_train, y_train, X_valid, y_valid)

    print("Evaluating on test set…")
    mean_pred, epistemic, aleatoric = predict_bnn(model, X_test, samples=100)

    test_rmse = np.sqrt(mean_squared_error(y_test, mean_pred))

    print(f"BNN Test RMSE: {test_rmse:.4f}")
    print(f"Epistemic Uncertainty (mean): {epistemic.mean():.6f}")
    print(f"Aleatoric Uncertainty: {aleatoric:.6f}")

    return test_rmse, epistemic, aleatoric, model

##add torch ANN
class AirbnbANN(nn.Module):
    def __init__(self, input_dim):
        super().__init__()
...
def train_ann(X_train, y_train, X_valid, y_valid, lr=1e-3, epochs=50, batch_size=512):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # -------------------------------------------------
    # 1. Scale numeric features (same logic as BNN)
    # -------------------------------------------------
    scaler = StandardScaler()
    scaler.fit(X_train)
    ...
    y_valid_t = torch.tensor(y_valid.values, dtype=torch.float32).view(-1,1).to(device)

    # -------------------------------------------------
    # 3. Build the ANN
    # -------------------------------------------------
    model = AirbnbANN(input_dim=X_train.shape[1]).to(device)
    ...
    for epoch in range(epochs):
        ...
        model.eval()
        with torch.no_grad():
            val_pred = model(X_valid_t).cpu().numpy()
        ...
        if epoch % 5 == 0:
            print(f"Epoch {epoch} | Validation RMSE: {val_rmse:.4f}")
    ...
    # Return scaler so you can scale the test set later
    return model, best_val_rmse, scaler


def evaluate_ann(model, X_test, y_test, scaler):
    model.eval()
    device = next(model.parameters()).device
...
def train_ann_old(X_train, y_train, X_valid, y_valid, lr=1e-3, epochs=50, batch_size=512):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0:
            print(f"Epoch {epoch:3d} | Validation RMSE: {val_rmse:.4f}")
...
def evaluate_ann_old(model, X_test, y_test):
    model.eval()
    device = next(model.parameters()).device
